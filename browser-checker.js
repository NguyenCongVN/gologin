...
const BROWSER_ARCHIVE_NAME = `orbita-browser-latest.${
...
class BrowserChecker {
  ...
  constructor(skipOrbitaHashChecking) {
    ...
    let executableFilePath = path.join(
      this.#browserPath,
      "orbita-browser",
      "chrome"
    );
  ...
  async checkBrowser(autoUpdateBrowser = false) {
    debug(`checking browser in ${this.#executableFilePath}`);
    const browserFolderExists = await access(this.#executableFilePath)
      .then(() => true)
      .catch(() => false);

    if (!browserFolderExists) {
      debug(`Browser folder does not exist! Download browser`);
    ...
    return new Promise((resolve) => {
      ...
      rl.question(
        `New Orbita ${browserLatestVersion} is available. Update? [y/n] `,
        (answer) => {
  ...
  async downloadBrowser() {
    ...
    debug("Orbita hash checked successfully");
    await this.replaceBrowser();
    await this.deleteOldArchives();
    debug("Orbita updated successfully");
  }
  ...
  downloadBrowserArchive(link, pathStr) {
    return new Promise((resolve, reject) => {
      ...
      writableStream.on("finish", () => {
        debug("Browser archive downloaded successfully");
        resolve();
      });
      ...
      const req = https.get(
        ...
        (res) => {
          ...
          const bar = new ProgressBar(
            "Orbita downloading [:bar] :rate/mps :downloadedMb/:fullMbMB :percent :etas",
            {
              complete: "=",
              incomplete: " ",
              width: 30,
      ...
      req.on("error", (err) => {
        debug("Error while downloading browser archive", err);
  ...
  async extractBrowser() {
    debug("Extracting Orbita");
    await mkdir(path.join(this.#browserPath, EXTRACTED_FOLDER), {
      recursive: true,
    });
    if (PLATFORM === "win32") {
      return decompress(
        path.join(this.#browserPath, BROWSER_ARCHIVE_NAME),
        path.join(this.#browserPath, EXTRACTED_FOLDER),
        {
    ...
    return exec(
      ...
      )} --directory ${path.join(this.#browserPath, EXTRACTED_FOLDER)}`
    );
  }
    ...
    await new Promise((resolve) =>
      https
        .get(
          ...
          (res) => {
            res.on("end", function () {
              debug("Hashfile downloading completed");
              writableStream.end();
              resolve();
            });
  ...
  async checkBrowserSum() {
    ...
    const hashFileContent = await exec(
      `cat ${path.join(this.#browserPath, DEB_HASH_FILE)}`
    );
    ...
    const calculateLocalBrowserHash = await exec(
      `cd ${path.join(
        this.#browserPath,
        EXTRACTED_FOLDER
      )} && find orbita-browser -type f -print0 | sort -z | \
            xargs -0 sha256sum > ${this.#browserPath}/calculatedFolderSha.txt`
    );

    const localHashContent = await exec(
      `cd ${this.#browserPath} && sha256sum calculatedFolderSha.txt`
    );
  ...
  async deleteDir(path = "") {
    ...
    const directoryExists = await access(path)
      .then(() => true)
      .catch(() => false);
    if (!directoryExists) {
      return;
